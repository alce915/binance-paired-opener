...
@pytest.mark.asyncio
async def test_subscribe_initial_state_uses_whitelist_default_symbol(tmp_path) -> None:
    gateway = SlowGateway()
    settings = api_module.Settings(
        _env_file=None,
        symbol_whitelist=["BTCUSDC", "ETHUSDC"],
        symbol_whitelist_file=tmp_path / "symbol_whitelist.json",
    )
    controller = MarketStreamController(gateway, settings=settings)
    ...
    assert [event["event"] for event in events] == ["connection_status", "account_overview", "execution_stats"]
    assert all(event["data"]["symbol"] == "BTCUSDC" for event in events)
    controller.unsubscribe(queue)
