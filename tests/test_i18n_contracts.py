...
def test_root_catalog_includes_simulation_page_display_messages() -> None:
    from app_i18n.runtime import format_copy, frontend_bootstrap_payload

    payload = frontend_bootstrap_payload(namespaces=("console", "runtime", "reasons", "events"))
    catalog = payload["messages"]

    required_keys = {
        "runtime.simulation_busy",
        "runtime.simulation_run",
        "runtime.simulation_running_click_abort",
        "runtime.simulation_run_interrupted",
        "runtime.simulation_stage.starting",
        "runtime.simulation_stage.running",
        "runtime.simulation_stage.waiting_fill",
        "runtime.simulation_stage.waiting_interval",
        "runtime.simulation_stage.finalizing",
        "runtime.simulation_stage.aborting",
        "runtime.simulation_stage.completed",
        "runtime.simulation_stage.interrupted",
        "runtime.simulation_lock_reason.simulation_running",
        "runtime.stop_reason.max_simulation_duration_reached",
        "runtime.stop_reason.interrupted",
        "runtime.execution_conflict_simulation_active",
        "runtime.execution_conflict_real_active",
        "console.action.abort",
        "console.action.monitor",
        "console.action.monitoring",
        "console.action.copy_to_real",
        "console.action.rerun_current_market",
        "console.simulation.positions_empty",
        "console.simulation.history_empty",
        "console.simulation.account_reset",
        "console.simulation.account_settings_saved",
        "console.stats.real_status_idle",
        "events.quote_stale",
        "events.extension_round_conflict_detected",
        "events.order_class.manual",
        "events.order_class.system",
    }
    assert required_keys.issubset(catalog)
    assert format_copy("runtime.simulation_busy") == "当前有模拟执行正在进行，请先终止或等待当前任务完成。"


def test_event_registry_covers_backend_session_events() -> None:
    import re

    from app_i18n.runtime import event_registry, messages

    project_root = Path(__file__).resolve().parents[1]
    emitted_events: set[str] = set()
    for relative_path in ("paired_opener/engine.py", "paired_opener/service.py"):
        source = (project_root / relative_path).read_text(encoding="utf-8")
        emitted_events.update(
            re.findall(r"add_event\(\s*[^,]+,\s*['\"]([^'\"]+)['\"]", source, re.DOTALL)
        )

    # Some execution events are built from a leg label at runtime. Keep the known
    # concrete labels in the contract so registry-only rendering never drops them.
    emitted_events.update(
        {
            "stage2_market_fallback",
            "close_stage2_market_fallback",
        }
    )

    registry = event_registry()
    catalog = messages()
    missing_events = sorted(emitted_events - set(registry))
    assert missing_events == []

    missing_message_keys = sorted(
        event_type
        for event_type in emitted_events
        if registry[event_type]["key"] not in catalog
    )
