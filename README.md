...
- 订单簿、SSE 推送、模拟执行
- 多账户切换
- “亢龙有悔”一次性多账号双腿开仓/平仓模拟：按安全凭据顺序逐账号执行，并记录手续费、磨损、恢复与配平状态
...
在控制台切换到“亢龙有悔”，通过“管理真实 API 账号”上传或维护 HMAC API Key。凭据管理仅允许本机回环访问，Secret 不会回显或写入浏览器持久化存储；导入必须先预览，再原子提交。

批次表单默认请求 `ETHUSDC`、`LONG` 优先、`100X`、每腿 `250000 USD`、30 轮和 3 秒间隔，所有参数均可修改并保存为默认值。开仓前会按账号展示双腿名义价值占保守可开仓位的百分比、当前/请求/有效杠杆、投影敞口、限制档位和快照时间；平仓按来源批次各账号的两腿剩余量规划，不复用开仓容量阻断。

当前阶段仅做真实 Binance 行情与账户数据驱动的本地模拟，网关不会提交订单、修改杠杆、修改持仓模式或撤单。任何未来真实下单实现都必须重新进行权限、限额、幂等、恢复和生产安全评审。
